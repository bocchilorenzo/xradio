  ...
  <div>
    ...
    >
      ...
      >
        ...
        <svg v-else class="fill-current h-6" viewBox="0 0 24 24">
          <path
            fill="currentColor"
...
async function search() {
  ...
  let url = "";
  initValues();
  if (route.params.mode == "country") {
    url = props.baseUrl + store.state.urls.search.countrycode + route.params.q;
  } else {
    url = props.baseUrl + store.state.urls.search.name + searchText.value;
  }
  await fetch(url, {
    method: "get",
    ...
    .then((res) => {
      if (res.length == 0) {
        ...
        if (store.state.selectedCountries.size > 0) {
          for (let i = 0; i < res.length; i++) {
            if (store.state.selectedCountries.has(res[i].countrycode)) {
              if (resultsShow.value.length < 15) {
                resultsShow.value.push(res[i]);
              }
              results.value.push(res[i]);
            }
          }
        } else {
          results.value = res;
          resultsShow.value.push(
            ...results.value.slice(startIndex.value, nextIndex.value)
          );
        }
...
function addResults() {
  ...
  if (last.value) {
    ...
    startIndex.value = nextIndex.value;
    if (nextIndex.value + 15 >= results.value.length) {
      nextIndex.value = results.value.length;
      last.value = true;
    } else {
      nextIndex.value += 15;
    }
...
async function filterResults(changed) {
...
if (route.params.mode == "text") {
  searchText.value = route.params.q;
}
search();
</script>
