    ...
    >
      <div class="flex flex-col p-4">
        ...
        <div class="py-2">
          ...
          <p class="pb-2">Choose countries</p>
          <div
...
let showCountries = ref(true);
let showQualities = ref(false);
let selectedCountries = ref(new Set());

selectedCountries.value = store.state.selectedCountries;
...
function toggleModal() {
  if (changed.value) {
    store.dispatch("setSelectedCountries", selectedCountries.value);
  }
...
function toggleCountries() {
  changed.value = true;
  if (selectedCountries.value.size != store.state.countries.length) {
    for (let i = 0; i < store.state.countries.length; i++) {
      selectedCountries.value.add(store.state.countries[i].iso_3166_1);
    }
  } else {
...
function changeCountrySelection(element) {
  changed.value = true;
  if (element.target.checked) {
    selectedCountries.value.add(element.target.value);
  } else {
    if (selectedCountries.value.has(element.target.value)) {
      selectedCountries.value.delete(element.target.value);
    }
  }
}
